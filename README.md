...
> 例：通过给出归谬证明来证明 $\squr{2}$ 是无理数。
> > 实数r是有理数，若存在整数p和q(q≠0,p,q互质)使得r=p/q。不是有理数的实数称为无理数。
> > 
> > 假设 $\squr{2}$ 为有理数则存在 $\squr{2}=\frac{p}{q}$ ,所以有 $2q^2=p^2$ ，此时说明p和q不互质，矛盾。
...
> > 证明，假设 $squr{2}^{\squr{2}}$ 分两种情况，为有理数和为无理数，若为有理数则得证，如果为无理数，则考虑 $(squr{2}^{\squr{2}})^{\squr{2}}$ 可知 $\squr{2}$ 和 $squr{2}^{\squr{2}}$ 均为无理数，但是 $(squr{2}^{\squr{2}})^{\squr{2}}$ 为有理数2，得证。
...
> 例:给定两个正实数a和b，其算术均值是(a+b)/2，几何均值是 $\squr{ab}$ ，证明算术均值总是大于几何均值。
> > 用均值不等式逆推
