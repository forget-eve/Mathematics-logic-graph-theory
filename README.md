...
> 例.请证明：对每个整数n，存在一个数是n的倍数，且在它的十进制表示中只出现0和1。

### 广义鸽巢原理
广义鸽巢原理（the generalized pigeonhole 
principle）：如果N个物体放入k个盒子，那么至少有
一个盒子包含了至少N/k个物体。
 把一些物体分到k个盒子中，需要确保某个盒子至少含有r个物
体：最少需N=k(r-1)+1
a)从一副标准的52张牌中必须选多少张牌才能保证
选出的牌中至少有三张是同样的花色；
b)必须选多少张才能保证选出的牌中至少有3张是红
心。
